  ...
  return (
    <section className="relative h-[32rem] overflow-hidden">
      {slides.map((slide, index) => (
        <div
          key={index}
          className={`absolute inset-0 transition-opacity duration-1000 ease-in-out bg-cover bg-center bg-no-repeat ${index === current ? 'opacity-100 z-20' : 'opacity-0 z-10'
            }`}
      ...
      <div className="absolute bottom-6 left-1/2 transform -translate-x-1/2 flex space-x-2 z-30">
        {slides.map((_, index) => (
          <div
            ...
            className={`h-3 w-3 rounded-full cursor-pointer ${current === index ? 'bg-white' : 'bg-gray-400'
              }`}
          />
        ))}
      </div>
